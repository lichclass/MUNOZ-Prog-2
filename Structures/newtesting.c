...
void printPassed(teacherRecord record){
    for (int i = record.passersList.count - 1; i >= 0; i--) {
        printf("Student %d:\n", record.passersList.count - i);
        printf("ID number: %d\n", record.passersList.studentList[i].idNum);
        printf("Name: %s %s\n", record.passersList.studentList[i].studentName.fName, record.passersList.studentList[i].studentName.lName);
        printf("Course: %s\n", record.classList.studentList[i].course);
        printf("Grades:\n");
        for (int j = 0; j < 3; j++) {
            printf("Subject %d: %s, Grade: %d\n", j + 1, record.passersList.studentList[i].studentGrade[j].subjectName, record.passersList.studentList[i].studentGrade[j].grade);
        }
        printf("\n");
    }
}
